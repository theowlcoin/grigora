...
#if defined(NDEBUG)
# error "Grigora cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6beddf5802303c855275a907257c518fa54a5d44c58fee171702b7dece90c17d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Grigora: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Grigora Signed Message:\n";
...
{
    // Grigora: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Grigora: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Grigora: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Grigora: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Grigora: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Grigora: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6beddf5802303c855275a907257c518fa54a5d44c58fee171702b7dece90c17d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "First test grigora";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1516479427;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1279692;
        ...
        assert(block.hashMerkleRoot == uint256("0x35107acdc9a659401f37cf119e228237823e2a86e1eb69d96a21b77505536a7c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Grigora: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GrigoraMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GrigoraMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GrigoraMiner(CWallet *pwallet)
{
    printf("GrigoraMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("grigora-miner");
    ...
    try { loop {
        ...
        printf("Running GrigoraMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GrigoraMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GrigoraMiner, pwallet));
}
